...
def parloops(patient_len):
    ...
    #    x_art_ph = pd.to_numeric(x_art_ph)
        ...
        ### Chartevents Vitals    
        df_pat.Heart_Rate_Score.iloc[index] = max(sc.HeartRate(min(x_hr)),sc.HeartRate(max(x_hr)));
        df_pat.Temperature.iloc[index] = max(sc.Temperature(min(x_temp)),sc.Temperature(max(x_temp)));
        df_pat.Repsiratory_Rate.iloc[index] = max(sc.RespiratoryRate(min(x_resp)),sc.RespiratoryRate(max(x_resp)));
        df_pat.White_Blood_Cells.iloc[index] = max(sc.WBC(min(x_wbc)),sc.WBC(max(x_wbc)));
    ...
    #    df_patients.Arterial_pH.iloc[index] = max(sc.ArterialpH(min(x_art_ph)),sc.ArterialpH(max(x_art_ph)));
        df_pat.Serum_Sodium.iloc[index] = max(sc.SerumSodium(min(x_sodium)),sc.SerumSodium(max(x_sodium)));
        df_pat.Serum_Potassium.iloc[index] = max(sc.SerumPotassium(min(x_potassium)),sc.SerumPotassium(max(x_potassium)));
        df_pat.Serum_Creatinine.iloc[index] = max(sc.SerumCreatinine(min(x_creatinine)),sc.SerumCreatinine(max(x_creatinine)));
        df_pat.Hematocrit.iloc[index] = max(sc.Hematocrit(min(x_hematocrit)),sc.Hematocrit(max(x_hematocrit)));
        df_pat.Serum_HCO3.iloc[index] = max(sc.SerumHCO3(min(x_hco3)),sc.SerumHCO3(max(x_hco3)));
