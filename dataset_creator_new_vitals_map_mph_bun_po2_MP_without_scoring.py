...
df_age_exp_all = pd.read_sql("SELECT SUBJECT_ID, AGE FROM PATIENTS_AGE", con=mimic_db_th)
df_age_exp_all.set_index("SUBJECT_ID", inplace=True)

df_exp_all = pd.read_sql("SELECT SUBJECT_ID, HOSPITAL_EXPIRE_FLAG FROM PATIENTS_AGE", con=mimic_db_th)
df_exp_all.set_index("SUBJECT_ID", inplace=True)
...
#df_map_all = pd.read_sql("SELECT ROW_ID, SUBJECT_ID, CHARTTIME, VALUENUM FROM CHARTEVENTS WHERE ITEMID=220052 ORDER BY ROW_ID ASC", con=mimic_db_th)
#df_map_all.set_index("SUBJECT_ID", inplace=True)


#### For LAB Events Vitals
df_art_ph_all = pd.read_sql("SELECT ROW_ID, SUBJECT_ID, CHARTTIME, VALUENUM FROM LABEVENTS WHERE ITEMID=50820 ORDER BY ROW_ID ASC", con=mimic_db_th)
...
df_po2_all = pd.read_sql("SELECT ROW_ID, SUBJECT_ID, CHARTTIME, VALUENUM FROM LABEVENTS WHERE ITEMID=50821 ORDER BY ROW_ID ASC", con=mimic_db_th)
df_po2_all.set_index("SUBJECT_ID", inplace=True)
...
def parloops(patient_len):
    ...
    for index in patient_len:
        ...
        df_exp = df_exp_all.loc[[patient_id]]
        expiry_flag = df_exp['HOSPITAL_EXPIRE_FLAG'].values
...
#        df_map = df_map[np.isfinite(df_map['VALUENUM'])]
        ...
        df_po2 = pd.DataFrame(df_po2_all.loc[[patient_id]])  
        df_po2 = df_po2[np.isfinite(df_po2['VALUENUM'])]        
        ### Chartevents Vitals
...
#        df_map.reset_index(inplace=True)
        ...
        df_po2.reset_index(inplace=True)
...
#        x_map = df_map[df_map['CHARTTIME']<=(df_map.CHARTTIME[0]+dt.timedelta(days=1))]['VALUENUM']
        ...
        x_po2 = df_po2[df_po2['CHARTTIME']<=(df_po2.CHARTTIME[0]+dt.timedelta(days=1))]['VALUENUM']
...
#        df_pat.Mean_Arterial_Pressure.iloc[index] = sc.MAP(min(x_map), max(x_map));
        df_pat.Mean_Arterial_pH.iloc[index] = sc.ArterialpH(min(x_art_ph), max(x_art_ph));
        df_pat.PO2.iloc[index] = sc.PO2(min(x_po2), max(x_po2));
