...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 29 17:31:38 2018
...
def parloops(patient_len):
    ...
    #    x_art_ph = pd.to_numeric(x_art_ph)
        ...
        ### Chartevents Vitals    
        df_patients.Heart_Rate_Score.iloc[index] = sum(x_hr)/len(x_hr)
        df_patients.Temperature.iloc[index] = sum(x_temp)/len(x_temp)
        df_patients.Repsiratory_Rate.iloc[index] = sum(x_resp)/len(x_resp)
        df_patients.White_Blood_Cells.iloc[index] = sum(x_wbc)/len(x_wbc)
    ...
    #    df_patients.Arterial_pH.iloc[index] = max(sc.ArterialpH(min(x_art_ph)),sc.ArterialpH(max(x_art_ph)));
        df_patients.Serum_Sodium.iloc[index] = sum(x_sodium)/len(x_sodium)
        df_patients.Serum_Potassium.iloc[index] = sum(x_potassium)/len(x_potassium)
        df_patients.Serum_Creatinine.iloc[index] = sum(x_creatinine)/len(x_creatinine)
        df_patients.Hematocrit.iloc[index] = sum(x_hematocrit)/len(x_hematocrit)
        df_patients.Serum_HCO3.iloc[index] = sum(x_hco3)/len(x_hco3)
        ## Add data of age and expiry
        df_patients.AGE.iloc[index] = age;
