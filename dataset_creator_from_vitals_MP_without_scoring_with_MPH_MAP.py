...
df_exp_all = pd.read_sql("SELECT SUBJECT_ID, HOSPITAL_EXPIRE_FLAG FROM ADMISSIONS", con=mimic_db_th)
...
def parloops(patient_len):
    ...
    for index in patient_len:
        ...
        if (sc.MAP(min(x_map)) > sc.MAP(max(x_map))): 
            df_pat.Mean_Arterial_Pressure.iloc[index] = min(x_map);
        else:
            df_pat.Mean_Arterial_Pressure.iloc[index] = max(x_map);

        if (sc.ArterialpH(min(x_art_ph)) > sc.ArterialpH(max(x_art_ph))): 
            df_pat.Mean_Arterial_pH.iloc[index] = min(x_art_ph);
        else:
            df_pat.Mean_Arterial_pH.iloc[index] = max(x_art_ph);
