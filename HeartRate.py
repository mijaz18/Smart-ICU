...
columns=['SUBJECT_ID', 'MAX_VALUE','MIN_VALUE']
ids=pd.read_sql("SELECT SUBJECT_ID, VALUE FROM CHARTEVENTS WHERE ITEMID=%(itemID)s",params={'itemID': item_id},con=mimic_db_demp)
de=ids
ids['MAX_VALUE']=0
ids['MIN_VALUE']=0
#ids.dr
...
for i in range(0, len(ids)):
     x= ids.iloc[i]['SUBJECT_ID']
     patient_id=x
     item_id = 220045
     chart_values = pd.read_sql("SELECT ROW_ID, CHARTTIME, VALUE FROM CHARTEVENTS WHERE SUBJECT_ID=%(id)s AND ITEMID=%(itemID)s", params={'id':patient_id,'itemID':item_id}, con=mimic_db_demp)
     df = chart_values
     x = df[df['CHARTTIME']<=(df.CHARTTIME[0]+dt.timedelta(days=1))]['VALUE']
     x = pd.to_numeric(x)
     max_val= max(x)
