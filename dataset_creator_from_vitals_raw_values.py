...
#    x_art_ph = pd.to_numeric(x_art_ph)
    ...
    if (sc.HeartRate(min(x_hr)) > sc.HeartRate(max(x_hr))): 
        df_patients.Heart_Rate_Score.iloc[index] = min(x_hr);
    else:
        df_patients.Heart_Rate_Score.iloc[index] = max(x_hr);

    if ( sc.Temperature(min(x_temp)) > sc.Temperature(max(x_temp))):
        df_patients.Temperature.iloc[index] = min(x_temp);
    else:
        df_patients.Temperature.iloc[index] = max(x_temp);

    if ( sc.RespiratoryRate(min(x_resp)) > sc.RespiratoryRate(max(x_resp)) ):
        df_patients.Repsiratory_Rate.iloc[index] = min(x_resp);
    else:
        df_patients.Repsiratory_Rate.iloc[index] = max(x_resp);

    if (sc.WBC(min(x_wbc)) > sc.WBC(max(x_wbc))):
        df_patients.White_Blood_Cells.iloc[index] = min(x_wbc);
    else:
        df_patients.White_Blood_Cells.iloc[index] = max(x_wbc);
    ...
    if (sc.SerumSodium(min(x_sodium)) > sc.SerumSodium(max(x_sodium))):
        df_patients.Serum_Sodium.iloc[index] = min(x_sodium);
    else:
        df_patients.Serum_Sodium.iloc[index] = max(x_sodium);
    
    if (sc.SerumPotassium(min(x_potassium)) > sc.SerumPotassium(max(x_potassium))):
        df_patients.Serum_Potassium.iloc[index] = min(x_potassium);
    else:
        df_patients.Serum_Potassium.iloc[index] = max(x_potassium);
    
    if (sc.SerumCreatinine(min(x_creatinine)) > sc.SerumCreatinine(max(x_creatinine))):
        df_patients.Serum_Creatinine.iloc[index] = min(x_creatinine);
    else:
        df_patients.Serum_Creatinine.iloc[index] = max(x_creatinine);
    
    if (sc.Hematocrit(min(x_hematocrit)) > sc.Hematocrit(max(x_hematocrit))):
        df_patients.Hematocrit.iloc[index] = min(x_hematocrit);
    else:
        df_patients.Hematocrit.iloc[index] = max(x_hematocrit);
        
    if (sc.SerumHCO3(min(x_hco3)),sc.SerumHCO3(max(x_hco3))):
        df_patients.Serum_HCO3.iloc[index] = min(x_hco3);
    else:
        df_patients.Serum_HCO3.iloc[index] = max(x_hco3);
