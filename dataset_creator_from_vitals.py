...
mimic_db_filter = create_engine("mysql+mysqldb://mimic:"+'mimic'+"@adcom.mmzzhh.com/mimic_filtered_b")
mimic_db_th = create_engine("mysql+mysqldb://mimic:"+'mimic'+"@adcom.mmzzhh.com/mimic_5_b")
...
#    x_art_ph = pd.to_numeric(x_art_ph)
    ...
    ### Chartevents Vitals    
    df_patients.Heart_Rate_Score.iloc[index] = max(sc.HeartRate(min(x_hr)),sc.HeartRate(max(x_hr)));
    df_patients.Temperature.iloc[index] = max(sc.Temperature(min(x_temp)),sc.Temperature(max(x_temp)));
    df_patients.Repsiratory_Rate.iloc[index] = max(sc.RespiratoryRate(min(x_resp)),sc.RespiratoryRate(max(x_resp)));
    df_patients.White_Blood_Cells.iloc[index] = max(sc.WBC(min(x_wbc)),sc.WBC(max(x_wbc)));
    df_patients.GCS.iloc[index] = sc.GCS(min(x_gcs_eye),min(x_gcs_motor),min(x_gcs_verbal));
#    df_patients.Mean_Arterial_Pressure.iloc[index] = max(sc.MAP(min(x_map)),sc.MAP(max(x_map)));
    ### LAB events Vitals
#    df_patients.Arterial_pH.iloc[index] = max(sc.ArterialpH(min(x_art_ph)),sc.ArterialpH(max(x_art_ph)));
    df_patients.Serum_Sodium.iloc[index] = max(sc.SerumSodium(min(x_sodium)),sc.SerumSodium(max(x_sodium)));
    df_patients.Serum_Potassium.iloc[index] = max(sc.SerumPotassium(min(x_potassium)),sc.SerumPotassium(max(x_potassium)));
    df_patients.Serum_Creatinine.iloc[index] = max(sc.SerumCreatinine(min(x_creatinine)),sc.SerumCreatinine(max(x_creatinine)));
    df_patients.Hematocrit.iloc[index] = max(sc.Hematocrit(min(x_hematocrit)),sc.Hematocrit(max(x_hematocrit)));
    df_patients.Serum_HCO3.iloc[index] = max(sc.SerumHCO3(min(x_hco3)),sc.SerumHCO3(max(x_hco3)));
